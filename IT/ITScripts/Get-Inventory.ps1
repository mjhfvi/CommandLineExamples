...
Foreach ($s in $servers)
{
    ...
    $ComputerInfo = Get-ComputerInfo -ComputerName $s
    ...
    .\Get-ProductKey.ps1
    $WindowsKey = Get-ProductKey -ComputerName $s                                                                                                          #run "Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass" & "Import-Module .\Get-ProductKey.ps1" & start Remote Registry service
    $CPUModel = Get-WmiObject -class "Win32_Processor" -namespace "root/CIMV2" -computername $s 
    ...
    $MotherboardInfo = Get-WmiObject Win32_BaseBoard -ComputerName $s
    ...
    $DISKType = Get-WmiObject win32_diskdrive -ComputerName $s
    foreach ($Disk in $DISKInfo) 
    { 
    ...
    $GPU = Get-WmiObject win32_VideoController -ComputerName $s
    #$IPAddress = Get-WmiObject -Class Win32_NetworkAdapter                                                                                                #Get NIC information
	...
	Foreach ($CPU in $CPUInfo)
	{
		...
		Add-Member -inputObject $infoObject -memberType NoteProperty -name "Motherboard Model" -value $MotherboardInfo.Product
        ...
        Add-Member -inputObject $infoObject -memberType NoteProperty -name "GPU" -value $GPU.name
